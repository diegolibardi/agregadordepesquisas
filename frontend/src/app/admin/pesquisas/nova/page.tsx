...
import { getInstitutes, getCandidates, adminCreatePoll } from "@/lib/api";
...
export default function NovaPesquisaPage() {
  ...
  useEffect(() => {
    setResults([{ candidate_id: 0, percentage: "", margin_of_error: "", is_spontaneous: false }]);
  }, [form.election_type]);
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return adminCreatePoll(payload);
    },
  ...
  const canSubmit =
    form.institute_id &&
    form.poll_date &&
    results.some((r) => r.candidate_id > 0 && r.percentage !== "");
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="space-y-6">
        ...
        <section className="bg-white rounded-xl shadow-sm border p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" className={inputCls} placeholder="ex: ES-00123/2026" {...field("tse_registered")} />
            </div>

            <div>
              <label className={labelCls}>URL da fonte</label>
              <input type="url" className={inputCls} placeholder="https://..." {...field("source_url")} />
            </div>

            <div className="sm:col-span-2">
              <label className={labelCls}>Notas (ex: derrubada pela JE, metodologia especial...)</label>
              <textarea
                rows={2}
                className={inputCls}
        ...
        <section className="bg-white rounded-xl shadow-sm border p-5">
          ...
          {!candidates?.length && form.election_type && (
            <p className="text-xs text-amber-600 mb-3">
              Nenhum candidato cadastrado para este tipo de eleição.{" "}
              <Link href="/admin/candidatos" className="underline">Cadastrar agora →</Link>
            </p>
          )}
              ...
              <div key={i} className="grid grid-cols-12 gap-2 items-end">
                ...
                <div className="col-span-3">
                  ...
                  <input
                    ...
                    onChange={(e) => setResult(i, { percentage: e.target.value })}
                  />
                </div>
                ...
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    className={inputCls}
                    placeholder="—"
                    value={row.margin_of_error}
                    onChange={(e) => setResult(i, { margin_of_error: e.target.value })}
                  />
                </div>
          ...
          <p className="text-xs text-gray-400 mt-3">
            "Esp." = pesquisa espontânea (sem lista de candidatos)
          </p>
        </section>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <Save className="w-4 h-4" />
            {save.isPending ? "Salvando..." : "Salvar Pesquisa"}
          </button>
          <Link
            href="/admin/pesquisas"
